...
Use one live input owner. F8, focus loss, and the persistent stop file interrupt input; they do not pause the game. Honor an intentional user interruption and do not reclaim focus from another app. A sequence timeout or visual mismatch alone does not require new user permission. When Minecraft is foreground and functioning normally, recover autonomously from the observed state, following [中断・エラーからの復旧](references/survival-ja.md#中断エラーからの復旧), including clearing a confirmed timeout stop when needed. Verify the actual pause and requested gameplay outcome before reporting them.
