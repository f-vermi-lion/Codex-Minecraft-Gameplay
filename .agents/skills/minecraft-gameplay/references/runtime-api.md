...
明るい炎だけでなく、溶岩に潜ったときの暗赤色の覆いも監視します。いずれも画面上の推定であり、未接触の溶岩やすべての表示条件を検知する保証はありません。

直前に確認した平坦な退路があり、採掘中の水平方向を固定できる場合に限り、`survival_watch.retreat_on_alert(raw, seconds=...)` を使えます。`hold_async` の**外側**に置くと、警告時に採掘入力の解放を待ち、同じMinecraftセッションからSで後退してポーズし、元の警告を再送出します。後退時間に歩ける距離以上の床と頭上を確認してください。未知の地形、曲がり角、GUI、旋回を含む操作群には使いません。フォーカス・入力境界などの例外では復帰入力を送りません。

```python
from survival_watch import retreat_on_alert

# 例: 水平の向きと後方約4ブロックの平坦な通路を確認済み。
with retreat_on_alert(raw, seconds=.8):
    with raw.hold_async(buttons=('left',), seconds=1.2) as mining:
        while not mining.done():
            watch.frame()
            raw.wait(.04)
        mining.result()
    watch.wait(.6)  # 掘り抜いた直後の流入も同じ退避範囲で監視。
```

溶岩中にいる場合は、封鎖位置の照準や所持品整理より退避を先にします。異常時の終了処理でも、診断用画像の保存より実際のポーズ確認を先にし、撮影・解析中に世界を進め続けないようにします。

`watch.last_frame` は、最後に検査した画像（警告の原因となった画像を含む）を保持します。後退やポーズが終わった後に保存でき、新しい撮影のために危険な場所で待つ必要はありません。撮影時点の画像なので、退避後の現在地とは区別します。
