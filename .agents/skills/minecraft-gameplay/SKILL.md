...
同一Minecraftセッションでは、入力保持と画面取得・画像解析を非同期・並行に行ってよい。`game.hold_async(...)` の管理されたスコープを使い、入力の所有者と停止・解放はruntimeへ委ねる。保持中の観察方法と終了時の扱いは [runtime-api.md](references/runtime-api.md) を参照する。

## 継続用の記録

セッションをまたぐ情報には、兄弟の `Codexs-obsidian-vault` を使える。再開時や記録時は、書庫の `AGENTS.md` とMinecraft用P文書から必要なノートだけをたどる。

`Minecraft攻略の現在地.md` は現在の再開導線であり、唯一の記録先や固定形式ではない。現在地、装備、危険、次の候補、確認済みの判断など、後のセッションに役立つ情報を残す。入力ログや定型項目を網羅する必要はなく、既存情報とリンクを保ちながら記録構造を改善してよい。
